...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_retail_item` AS select `a`.`id` AS `id`,`a`.`retail_id` AS `retail_id`,`a`.`medicine_id` AS `medicine_id`,`b`.`sn` AS `medicine_sn`,`b`.`name` AS `medicine_name`,`b`.`spec` AS `medicine_spec`,`b`.`unit_name` AS `unit_name`,`a`.`medicine_price` AS `medicine_price`,`a`.`medicine_num` AS `medicine_num`,`a`.`retail_price` AS `retail_price` from (`retail_item` `a` left join `view_medicine` `b` on((`a`.`medicine_id` = `b`.`id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2013-05-04 20:40:05
