...
static LPVOID			g_vpParam		= NULL;

#define	MD5_BUFFER_SIZE		(60l * 1024l)
...
DWORD MD5String(IN char* szString, OUT uchar* pbymd5)
{
	if (!szString || !pbymd5)
	{
	...
	memcpy(pbymd5, alg.Digest(), 16);

	return ERROR_SUCCESS;
}

// MD5Buffer: Performs the MD5 algorithm on a buffer,
DWORD MD5Buffer(IN BYTE* pbuf, IN DWORD dwBufSize, OUT uchar* pbymd5)
{
	if (!pbuf)
	{
		return ERROR_INVALID_PARAMETER;
